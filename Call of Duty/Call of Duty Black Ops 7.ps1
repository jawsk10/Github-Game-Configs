...
Get-FileFromWeb -URL "https://github.com/FR33THYFR33THY/Github-Game-Configs/raw/refs/heads/main/Call%20of%20Duty/Call%20of%20Duty%20Black%20Ops%207.zip" -File "$env:TEMP\BO7.zip"
Clear-Host

# extract config files
Expand-Archive "$env:TEMP\BO7.zip" -DestinationPath "$env:TEMP\BO7" -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
Copy-Item -Path "$env:TEMP\BO7\players\*" -Destination "$env:LocalAppData\Activision\Call of Duty\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:TEMP\BO7" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:TEMP\BO7.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
